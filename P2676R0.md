...
safely in Val were we to add Rust-style named lifetime annotations.  These annotations, however, are
so notoriously difficult to use that “fighting the borrow checker” has become a meme among Rust
programmers. At the same time, because Val exploits the semantics of whole/part relationships, we
believe it can safely express far more than lifetime-annotation-free Rust.  The Val object model is
in a “sweet spot” of simplicity and expressive power.
...
The Val object model might not be the right thing for C++, but 
