...
void offset_twice(BigNum& x, BigNum delta) {
  x += delta
  x += delta
}
...
```cpp
// Offsets x by 2*delta.
void offset_twice(BigNum& x, BigNum const& delta) {
  x += delta
  x += delta
}
...
void main() {
  BigNum x = 3;
  offset_twice(x, BigNum(x));  // Note explicit copy
  std::cout << x << std::endl; // Prints 9
}
```

Of course, independence requirements are almost never stated explicitly: in practice, there is an
*implicit Law of Exclusivity* [@SE-0176], which requires the values of any object denoted by a
mutable reference to be independent of the values of all other variables.  We claim this law is
built into every C++ programmer's mental model, because there is no other rational way to deal with
mutation. 

In fact, the semantics of a mutating function is nearly impossible to specify unless the Law is
upheld.  In response to specific problems discovered over decades, you the standard bans a variety
of *specific violations* (e.g. [27.2
algorithms.requirements¶7](https://eel.is/c++draft/algorithms.requirements#7)), but it is still easy
to find cases like this one, which is required to work by the standard, but can't possibly do so and
causes undefined behavior in practice ([godbolt](https://godbolt.org/z/c55GoM9dY)):
...
Or this one, whose behavior is at best implicitly unspecified
