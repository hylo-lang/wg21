...
## Val Parameter Passing Conventions
...
| Val declaration     | Semantics             | C++ mechanics         | Static guarantee/Note                 |
|---------------------|-----------------------|-----------------------|---------------------------------------|
| `x: T` / `x: let T` | pass-by-`const`-value | `T const&`            | Referent is truly immutable           |
...
independent value: it cannot mutate, either directly through `x` or by any other means.

Mutation in Val is always via `inout`. Semantically, the parameter is an independent value wholly
owned by the callee and only accessible via `x`. This guarantee is enforced at the call site by
disallowing (potentially) overlapping accesses. Note that **mutations in Val are marked with `&`**
at the call site:

```swift
swap(&x.a, &x.b)     // OK if a and b are guaranteed distinct.
v.remove(v[2])       // ERROR: v, being mutated, overlaps v[2]. Copy v[2] first.
swap(&x[i], &x[j])   // ERROR: use index_swap to ensure `i != j`.
index_swap(&x, i, j) // OK; i == j can be checked for dynamically.
```
...
```swift
consume(x)
print(x)   // ERROR: x's lifetime ended when it was consumed above.
```

The argument is not moved until and unless it escapes the callee (i.e. is stored or returned).
...
**Law of Exclusivity** (LOE) [@SE-0176], which requires the values of any object denoted by a mutable
