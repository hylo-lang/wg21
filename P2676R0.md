...
Type safety can be achieved by avoiding unsafe casts and type puning while bounds safety—the freedom from out-of-bounds accesses—can be achieved with cheap dynamic checks.
...
Although the C++ Core Guidelines do not claim complete memory safety and do not address thread safety yet, we are confident it can reach these goals through iterative refinements.
This assumption is justified by the fact that they take the same path as Rust, which already offers statically guaranteed memory and thread safety by construction.
...
Using Rust as a proxy for a future more complete version of the C++ Core Guidlines, we can predict that Val's expressiveness is on par with the latter.
