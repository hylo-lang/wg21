...
| Val declaration     | Semantics             | C++ mechanics     | Static guarantee                      |
|---------------------|-----------------------|-------------------|---------------------------------------|
...
| `x: sink T`         | pass-by-move          | `T(std::move(x))` | Lifetime of argument ends at call     |
...
disallowing (potentially) overlapping accesses. Note that mutations in Val are marked with `&` at
the call site:
...
index_swap(&x, i, j) // OK; i != j can be dynamically checked.
```

The `sink` convention in Val denotes consumption of the argument; it's what you'd use for 
