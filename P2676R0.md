...
```cpp
// Offsets x by delta.
void offset(BigNum& x, BigNum delta) {
  x += delta;
}
...
```cpp
// Offsets x by delta.
void offset(BigNum& x, BigNum const& delta) {
  x += delta;
}
...
The only way to make the *After* code correct is to add an aditional independence requirement, and
hope that users uphold it.

```cpp
// Offsets x by delta. Requires: x and delta are distinct objects.
void offset(BigNum& x, BigNum const& delta) {
  x += delta;
}
...
void main() {
  BigNum x = 3;
  offset2(x, BigNum(x));       // Explicit copy
  std::cout << x << std::endl; // Prints 9
}
```

Of course, this independence requirement is almost never stated: in practice, there is an *implicit
Law of Exclusivity* [@SE-0176] which says that mutable references denote independent objects from
other accessible names.  In response to problems reported over decades of real-world use, the
standard contains a patchwork of language banning specific *ways* this law might be violated,
without ever requiring the law itself to be upheld.  As a result it is replete with cases like this
one, which can't possibly work but are nonetheleess required to
([godbolt](https://godbolt.org/z/c55GoM9dY)):
...
int main() {
  vector v = { 0, 1 };
  ranges::sort(
    v, [&](auto x, auto y){ v.push_back(v[0]); return y < x; });
}
```

Or this one, whose behavior is at best unspecified ([godbolt](https://godbolt.org/z/vnn49YTqb)):
