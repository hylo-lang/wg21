...
safely used in a different context.  This covers many cases that in Rust would require named
lifetime annotations. A less general but perhaps more recognizable set of use cases correspond to
places we'd be tempted to use proxy references in C++, if only they didn't cause so many problems:
